var expect = require( "expect.js" ),
    ...
    expectNoBrokenLinks = function( indicesToSkip ){
        var idMap = {},
            ...
            expectAllTankLinksAreValid = function( indicesToSkip ){
                for( var i = 0, max = test.tanks.length; i < max; i++ ){
                    if( !( indicesToSkip && indicesToSkip.indexOf( i ) > -1) ){
...
describe( "Testing Live Fish Tank functionality", function(){
    ...
    describe( "Deleting multiple tanks from 3x3 grid", function(){
        ...
        it( "removing eight and ninth leaves no unbroken links and correct metadata", function(){
            var removed = [8,7];
            test.server.removeFishTankWithSocket( test.sockets[removed[0]], function(){
                test.server.removeFishTankWithSocket( test.sockets[removed[1]], function(){
                    refreshTestTanks();
    ...
    describe( "Re-adding to a 5x5 grid", function(){
        ...
        it.only( "delete and re-add two consecutive tanks from each position", function(){
            for( var removedIndex = 0, numberOfTanks = 25; removedIndex < numberOfTanks - 1; removedIndex++ ){
                test.server.removeFishTankWithSocket( test.sockets[removedIndex], function(){
                    test.server.removeFishTankWithSocket( test.sockets[removedIndex + 1], function(){
                        test.server.addFishTankWithSocket( test.sockets[removedIndex], function(){
                            test.server.addFishTankWithSocket( test.sockets[removedIndex + 1], function(){
                                refreshTestTanks();
                                expectNoBrokenLinks();
                            } );
                        } );
